...
export const TokenRow = ({ token, isHovered, setHoveredIndex, toggleOpen, index, eventProperties }: TokenRowProps) => {
  ...
  const handleClick = useCallback(() => {
    ...
    address && addRecentlySearchedAsset({ address, chain: 'MODE' as Chain })

    toggleOpen()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [addRecentlySearchedAsset, token, toggleOpen, eventProperties])

  const tokenDetailsPath = getTokenDetailsURL({ address: token.address.toLowerCase(), chain: 'MODE' as Chain })
